...
int Lw(int core_no, string reg1, string reg2, int reg2_val, int offset, int line_no);
int Sw(int core_no, string reg1, string reg2, int reg1_val, int reg2_val, int offset, int line_no);
...
int compute(int core_no, instruct ins){
	...
	if(ins.ins_id == 1){
		...
		if(queue_length == 1){
			...
			Lw(core_no, ins_queue[0].reg[0], ins_queue[0].reg[1], ins_queue[0].reg2_val, ins_queue[0].val, ins_queue[0].line_no);
			return ins.line_no + 1;
		}
		if(CPU_CORES.at(core_no).on_hold == true && running_cycle == CPU_CORES.at(core_no).max_cycle){
			execute_queue(core_no, ins.ins_id);			
		...
		if(queue_length == 1){
			...
			Sw(core_no, ins_queue[0].reg[0], ins_queue[0].reg[1], ins_queue[0].reg1_val, ins_queue[0].reg2_val, ins_queue[0].val, ins_queue[0].line_no);
			return ins.line_no + 1;
		}
		if(CPU_CORES.at(core_no).on_hold == true && running_cycle == CPU_CORES.at(core_no).max_cycle){
...
int Lw(int core_no, string reg1, string reg2, int reg2_val, int offset, int line_no){
	...
	if(reg2_val + offset + CPU_CORES.at(core_no).start_address >= CPU_CORES.at(core_no).end_address){
		...
		if (cur_row == -1){
			...
			CPU_CORES.at(core_no).onhold_output =  /*"Core: " + std::to_string(core_no) +*/ "cycle " + std::to_string(CPU_CORES.at(core_no).prev_clock_cycles + 1) + "-" + std::to_string(CPU_CORES.at(core_no).clock_cycles) + ": " + reg1 + "="  + std::to_string(CPU_CORES.at(core_no).reg_values[r1])+ ", Row buffer not updated"+ ", Accessed address " + std::to_string(x) + " from row buffer\n\n";
			CPU_CORES.at(core_no).max_cycle = CPU_CORES.at(core_no).clock_cycles;
			CPU_CORES.at(core_no).clock_cycles = CPU_CORES.at(core_no).prev_clock_cycles;
			for(int p = 0; p < 32; p++){
				CPU_CORES.at(core_no).prev_reg_values[p] = CPU_CORES.at(core_no).reg_values[p];
			...
			CPU_CORES.at(core_no).onhold_output =  /*"Core: " + std::to_string(core_no) + */"cycle " + std::to_string(CPU_CORES.at(core_no).prev_clock_cycles + 1) + "-" + std::to_string(CPU_CORES.at(core_no).clock_cycles) + ": " + reg1 + "="  + std::to_string(CPU_CORES.at(core_no).reg_values[r1])+ ", Row buffer not updated" + ", Copied a row and activated row " + std::to_string(row) + " and accessed address " + std::to_string(x) + "\n\n";
			CPU_CORES.at(core_no).max_cycle = CPU_CORES.at(core_no).clock_cycles;
			CPU_CORES.at(core_no).clock_cycles = CPU_CORES.at(core_no).prev_clock_cycles;
			buffer_updates = buffer_updates + 1;
			for(int p = 0; p < 32; p++){
...
void execute_queue(int core_no, int id){
	...
	if(cur_core != core_no){
		if(running_cycle < CPU_CORES.at(cur_core).max_cycle){
			//cout << "\e[1m" << "\e[34m"<< "Core: " << core_no + 1 << "\e[0m" << "\e[0m"<< endl;
			cout << "Waiting for DRAM completion\n" << endl;
			return;
		}
	...
	if(queue_length > 1){
		...
		if(ins_queue[0].ins_id == 8){
			Lw(cur_core, ins_queue[0].reg[0], ins_queue[0].reg[1], ins_queue[0].reg2_val, ins_queue[0].val, ins_queue[0].line_no);
		}
		else{
			Sw(cur_core, ins_queue[0].reg[0], ins_queue[0].reg[1], ins_queue[0].reg1_val, ins_queue[0].reg2_val, ins_queue[0].val, ins_queue[0].line_no);
		}
	}
	else{
		CPU_CORES.at(cur_core).max_cycle = -1;
